...
THEFT_THRESHOLD = 0.9 
SEQUENCE_LENGTH = 90
OUTPUT_DIR = os.path.join(current_dir, "suspect_clips")
...
class PersonTheftAnalyzer:
    def __init__(self, track_id):
        ...
        self.detector = LSTMTheftDetectorInference(
            model_path=os.path.join(current_dir, 'Behaveioral/model/theft_detector_lstm.keras'),
            scaler_path=os.path.join(current_dir, 'Behaveioral/scaler/scaler_lstm.pkl'),
...
def run_theft_detection_pipeline(video_path):
        ...
        if results and results[0].boxes and results[0].boxes.id is not None:
            ...
            for box, track_id in zip(boxes, track_ids):
                ...
                if pose_results.pose_landmarks:
                    ...
                    if analyzer.history:
                        avg_prob = sum(analyzer.history) / len(analyzer.history)
                    else:
                        avg_prob = 0.0
                ...
                (text_w, text_h), baseline = cv2.getTextSize(label_text, cv2.FONT_ITALIC, 1.0, 2)
    ...
    for track_id in suspect_ids:
        ...
        c_out = cv2.VideoWriter(clip_path, cv2.VideoWriter_fourcc(*'mp4v'), 30.0, (w_c, h_c))
...
if __name__ == "__main__":
    video_path = "../Data/Stream/TRY/1.mp4"
    run_theft_detection_pipeline(video_path)
